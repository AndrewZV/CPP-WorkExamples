...
void BaseEngine::CreateObjectArray(int iNumberObjects)
{
	...
	displayableObjectsVector = {};			//Initialise empty vector
}
...
void BaseEngine::StoreObjectInArray(int iIndex, DisplayableObject* pObject)
{
	...
	if (!displayableObjectsVector.empty())
		displayableObjectsVector.pop_back();			//If the vector is not empty, pop the last element (NULL)

	displayableObjectsVector.push_back(pObject);		//Push object into vector
	if (pObject != NULL)
		displayableObjectsVector.push_back(NULL);			//Push NULL to end of vector
}
...
void BaseEngine::DestroyOldObjects()
{
	...
	m_iDrawableObjectsChanged = 1;
	if (displayableObjectsVector.empty() == false)
	{
		displayableObjectsVector.clear();	//destroy all objects in vector
	}
	displayableObjectsVector = {};
}
...
void BaseEngine::UpdateAllObjects( int iCurrentTime )
{
	...
	if (displayableObjectsVector.empty() == false)
	{
		for (std::vector<DisplayableObject*>::iterator it = displayableObjectsVector.begin(); it != displayableObjectsVector.end(); it++)
		{
			if (*it != NULL)
			{
				(*it)->DoUpdate(iCurrentTime);
				if (m_iDrawableObjectsChanged)
					return; // Abort! Something changed in the array
			}
		}
...
void BaseEngine::DrawObjects()
{
	...
	if (displayableObjectsVector.empty() == false)
	{
		for (std::vector<DisplayableObject*>::iterator it = displayableObjectsVector.begin(); it != displayableObjectsVector.end(); it++)	//Iterate through vector
		{
			if (*it != NULL)
			{
				(*it)->Draw();
...
DisplayableObject* BaseEngine::GetDisplayableObject( int iIndex )
{
	...
	if (displayableObjectsVector.empty() == false)
	{
		return displayableObjectsVector.at(iIndex);
	}
	else
		return NULL;
}
...
void BaseEngine::DestroyObject(int iObjectIndex)
{
	displayableObjectsVector.erase(displayableObjectsVector.begin() + iObjectIndex);
	if (displayableObjectsVector.back() != NULL)
	{
		displayableObjectsVector.push_back(NULL);
	}
	displayableObjectsVector.shrink_to_fit();
	DrawableObjectsChanged();
}


DisplayableObject* BaseEngine::getFirstElementInVector()
{
	return displayableObjectsVector.front();
}

int BaseEngine::getObjectId(DisplayableObject* thisObject)
{
	DisplayableObject* pObject;
	int thisObjectId = -1;

	for (int iObjectId = 0;
		(pObject = this->GetDisplayableObject(iObjectId)) != NULL;
		iObjectId++)
	{
		if (pObject == thisObject)
			thisObjectId = iObjectId;
	}
